...
# R-38「pk/test/compat/QtGlobal 让位守卫」的 transition TU 证据：三个模式都 exit 0 才算过。
#
# 用法：bash pk/test/tests/transition_qt_probe.sh
...
PK_TEST_COMPAT_DIR="$(cd "$SCRIPT_DIR/../compat" && pwd)"
PROBE="$SCRIPT_DIR/transition_qt_probe.cpp"
TMP_NOQT=/tmp/transition_test_noqt_$$
TMP_CORELIB=/tmp/transition_test_corelib_$$
trap 'rm -f "$TMP_NOQT" "$TMP_CORELIB"' EXIT

echo "== 模式 A：real Qt + kistest.h（经 compat/QObject 拉 compat/QtGlobal）同 TU =="
g++ -fPIC -std=gnu++17 -fsyntax-only "$PROBE" \
  -isystem "$QT/include" -isystem "$QT/include/QtCore" -isystem "$QT/include/QtGui" \
  -DQT_CORE_LIB -DQT_GUI_LIB -I "$PK_TEST_COMPAT_DIR"
r=$?
if [ $r -ne 0 ]; then echo "模式 A 编译失败（exit=$r）" >&2; exit $r; fi
echo "模式 A OK"

echo "== 模式 B：无 Qt =="
g++ -fPIC -std=gnu++17 "$PROBE" -o "$TMP_NOQT" -I "$PK_TEST_COMPAT_DIR"
r=$?
...
echo "== 模式 C：QT_CORE_LIB 定义 + 无真 Qt 头（守卫第二析取 !QGLOBAL_H）=="
g++ -fPIC -std=gnu++17 "$PROBE" -o "$TMP_CORELIB" -DQT_CORE_LIB -I "$PK_TEST_COMPAT_DIR"
r=$?
