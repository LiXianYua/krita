...
const double kBand[] = {
    ...
    // 非有限档（终审 B-1 / 修复轮 3 F1 那条路）：`arcMoveTo` 对角度**没有入口守卫**，
    // `±inf` / `NaN` 会一路走到 `pkFindEllipseCoords` 的 `int(t)`（上面那条折角对
    // 非有限值取真、而 `fmod(inf, 360) == NaN`）。这三格就是为常驻守它而加 ——
    // 不加，闸门对 B-1 全程瞎（修复前实测：`arcMoveTo(nan)` 触发 `:288:28` UB）。
