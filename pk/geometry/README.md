...
用例覆盖全部 **17** 个负载类型（本目录 8 个 + `pk/time` 3 个 + S-17 的
...
给本目录 8 个 + `pk/time` 3 个补挂属性前是 `6 passed, 11 failed`（失败项正好是这
11 个未挂属性类型）；裁决 A 给 `class PkVariant` 也挂上后，17 个类型全绿
（`20 passed, 0 failed`）。
...
| `PkPathClipper_p.h` / `PkPathClipper.cpp.inc`（**R-39 T1**） | 从 Qt 5.15.7 `qtbase/src/gui/painting/qpathclipper_p.h` 与 `qpathclipper.cpp` 移植的私有 winged-edge 路径布尔引擎；保留原始 Qt LGPL-3.0/GPL-2.0/GPL-3.0 license block。`PkPainterPath.cpp` 里的 `PkArcBezier` 辅助算术另来自同版本 `qbezier_p.h` / `qbezier.cpp`，而 close 后延迟 MoveTo 及 fuzzy 终点归一来自 `qpainterpath_p.h` 的 `QPainterPathData::close/maybeMoveTo`；末元素坐标编辑后的 current-position 同步和未闭合 `addPolygon` 构造则对齐 `qpainterpath.cpp` 的对应公开成员。文件头登记了 Qt Copyright、原路径与同一组商业/LGPL-3.0/GPL-2.0/GPL-3.0 许可选项；`pointAt` 保持上游 de Casteljau 求值顺序。`PkPainterPath.cpp` 单点 include `.inc`，公开 `united` / `intersected` / `subtracted` / `simplified`、路径 `contains` / `intersects` 及 `& | + -` 别名。类型替换只涉及 QPainterPath/QPointF/QLineF/QRectF/QBezier 与私有容器，核心拓扑/交点/绕数决策保持上游形态。 |
...
**④ 逐族 `-` 数棘轮（F7）**：map 里写 `-` 是「本对拍不含该重载的 `rec()`」的**显式分类**，
闸门③跳过它、摘要行里只计数。它**对任何族都可用** —— 别处新加一条声明、随手写个 `-`，
闸门照样放行，等于把一个覆盖缺口**静默**登记进来。棘轮把现状钉死：`run_oracle.sh` 的
`DASH_MAX` 给**每一族**一个 `-` 行数**上限**，顺序与 `API_GROUPS` **逐行对齐**；解析器先校验
**两表等长**（加族忘了加棘轮值时当场 FAIL，不会静默按 0 处理）。**某族的 `-` 计数 > 它的上限
即 FAIL**（指名族与两个数）—— 它只在「有新声明被登记成 `-`」时触发，因为新声明必然让该族
...
| 28 | **R-87：`PkRect` 新增一个 `std::ostream operator<<`（`pk/` 全树第一处，此前实测零处），文本 = 真 Qt `QTest::toString(const QRect&)` 的原文，只把类型名换成 `PkRect`** | **值文本本身是对齐 Qt，不是偏离**：真 Qt 的 `QCOMPARE(QRect,…)` 确实打值（`QtTest/qtest.h:168-173` 的逐类型特化，探针① 实测 `QRect(1,2 3x4) (bottomright 3,5)`）。偏离在**通道的落位**：真 Qt 把这条文本放在**测试库** `QtTest/qtest.h` 里，本仓放在**类型所有者** `pk/geometry` 里 —— 理由是 `pk/test/PkTestCompare.h:77-82` 的既有设计就写着「SFINAE 检测 ostream 可插入性，不需要为每个类型写重载」，于是生产者落类型所有者、`pk/test` 一个字节都不用改（`pk/test` 当时被 R-82 占着，这是硬约束）。**⚠ 与 `PkGeometryDebug.cpp:107` 的 `PkDebug operator<<` 是两条通道、两种文本，刻意不统一**：真 Qt 本身也是两条（`QTest::toString` 带 `(bottomright …)`、`QDebug` 不带），实测 `dbg << rect` 仍选中 `PkDebug` 那条自由运算符、文本一字不漂（`test_pkgeometry_debugstream` 全绿）。**⚠ 连带纪律（与偏离 11 对 `PkSize.cpp` 的 `<type_traits>` 同型）**：`PkRect.h` 从此带上它**唯一一条系统头** `#include <iosfwd>`，所以**把本头 include 进某个 namespace 的 TU（`oracle/*_difftest.cpp` 的 `namespace pkoracle`、`tests/*_macro_proof.cpp` 的匿名 namespace）必须把 `<iosfwd>` 先在 namespace 之外 include 一次**，否则会造出 `(anonymous)::std` / `pkoracle::std` 遮住 `::std`（实测形态：`reference to 'std' is ambiguous`）。本仓按该纪律修了 `tests/rectf_macro_proof.cpp`（系统头区新增一行），`PkRect.h` 与它各留一条注释指路。**判据**：`tests/comparable_diagnostic_test.cpp` 造一条故意失败的 `PK_COMPARE(PkRect…)`、截获 stdout、断言含两个实际值且不含 `<unprintable>`，**并由 `tests/run_tests.sh` 跑**（判据在收尾路径上）。 |
...
- **判别力（R-58 实测）**：注入 A（删折角行）红 `PP::arcTo band/int-conv` 与 `band/theta-loss`
  ...
  **修复轮 3 补**：注入 E（把 `pkQuadrantOf(t)` 还原成裸的 `int(t)`）UBSan 闸门 `exit=134`
  ...
  （`:308`）—— 证明补进 `kBand` 的非有限那三格确实咬住 B-1（改前对这三格是瞎的）。

- **「`sum` 那两条语料」没牙（R-58 修复轮 4 登记、修复轮 5 按复评实测订正；**不是**「已覆盖」）**：
  折角条件 `pkArcAngleUnsafe(startAngle) || pkArcAngleUnsafe(startAngle + sweepLength)` 对
  **这两个输入**（`{2147483600,360}`、`{2147483000,720}`）**整条都不起作用** ——
  实测三个变体（V1 整行去掉 / V2 只留第二个析取项 / V3 只留第一个析取项）
  **对这两条全部 SAME、全部绿**。机理：折角只可能在两处改值 ——
  `pkArcSegmentOf` 的夹取（`|sa| >= 2^31 * 90 = 1.93e11` 起）与
  `qreal(startSegment) * 90` 不再精确（`|sa|` 超过 `2^53` 之后）；
  这两个输入的 `|sa| ≈ 2.15e9`，两处都不沾 ⇒ 折角与否取值逐位相同
  （实测 `arcTo(2147483600,360)` 与 `arcTo(80,360)` 的 18 个元素 / type / 坐标（`%.17g`）/
  bounds 逐位相同）。
  ⇒ 它们**不是判别力证据**，只是把「`startAngle` 安全、`sa + sl` 越界」这条路径
  **留在语料里**；**不要**指望它们抓折角条件的回归。
  **折角本身的牙在别处**：注入 A（整条删折角行）红的是 `band/int-conv` 与 `band/theta-loss`
  —— 那些输入的 `|sa| >= 1.93e11`，夹取真的会改值。
  （修复轮 4 已把 `sa + sl` 的值嵌进 `sum` 两条的 tag，使将来真红时能归因到是哪一条；
  但**这不改变它们今天没牙**这个事实。）
...
- **浮点→int 越界（`int(inf)`、`int(2147483648.0)`）是另一类 UB，`-fwrapv` 管不着。**
  实机上运行期两侧编成同一条指令，取值一致；但**编译期常量折叠给的是另一个答案**，
  所以留在单测里的那几条一律经 `noFold()`（一次 `volatile` 读）把转换压到运行期
  （现在只剩 `sizefDivision` 里"浮点除以 0 得 ±inf"那两条 —— 那是**浮点**除零，
  IEC 559 定义良好，不是 UB，也不需要屏障）。
- **⚠ 「换平台要重测，点名 Android / ARM」——已在 macOS arm64 上重测（S-18，2026-09-11），
  结论是这条预警完全说对了，处置已按裁决改掉。** 实测：x86 的 `cvttsd2si` 越界返回
  "整数不定值" `INT_MIN`，而 **AArch64 的 `fcvtzs` 是饱和语义**（正越界给 `INT_MAX`、
  `nan` 给 0）。于是 `int(inf)` / `int(2147483648.0)` 这批断言在 arm64 上**基线即红**
  （实测 4 个测试函数、9 条断言）。
  **但 pk 没有错**：同一份枚举，一边跑真 Qt 5.15.7（arm64 framework）、一边跑本树的
  `libpkgeometry.a`，**20 行值逐行相同**（`+inf`、`2147483648.0`、`1e308`、
  `QPoint(1,1)/0.0`、`QSize(10,20)/0.0` 两侧**都**得 `2147483647`）。
  偏离的只是**写死在断言里的那个数** —— 它是某平台 UB 的观测值，不是规格。
  **处置（`$PK/docs/superpowers/specs/S线-spec.md`「平台相关的测试期望值：改成与真 Qt
  运行期对拍」，人拍板 C）**：这批断言**整块移出单测**，覆盖改由
  `oracle/geometry_difftest.cpp` 的 `F::toPoint` / `SF::toSize` rec()（同一批输入、
  tag 档位 `out-of-int-range`）**与真 Qt 当场对拍**承担。
  **判据因此从「两边各自跑出来的数是否相同」换成「真 Qt 与 pk 当场对拍是否一致」**
