...
- **「`sum` 那两条语料」没牙（R-58 修复轮 4 如实登记，**不是**「已覆盖」）**：
  折角条件 `pkArcAngleUnsafe(startAngle) || pkArcAngleUnsafe(startAngle + sweepLength)` 的
  **第二个析取项在现有代码上是可观测空操作** —— 实测 `arcTo(2147483600, 360)` 与
  `arcTo(80, 360)`（`80 == fmod(2147483600,360)`）的 **18 个元素 / type / 坐标（`%.17g`）/
  bounds 逐位相同**。机理：`startAngle` 在安全区间内 ⇒ 段索引 ≤ ~2.4e7 ⇒
  `qreal(startSegment) * 90` 精确 ⇒ 折角与否取值逐位相同。因此 `sum` 那两条语料
  （`{2147483600,360}`、`{2147483000,720}`）**红不了**，它们**不是判别力证据**，只是把
  「`startAngle` 安全、`sa + sl` 越界」这条路径**留在语料里**；**它们有条件的牙**：若将来有人把
  折角条件的**第一个**析取项也去掉，它们就会红。**本任务的判别力因此由注入 A / C / D 与 UBSan
  闸门承担，不由 `sum` 承担。**
  （修复轮 4 已把 `sa + sl` 的值嵌进 `sum` 两条的 tag —— `.../sum` 接 `ptok("v", c.sa + c.sl)`
  —— 使将来真红时能归因到是哪一条；但**这不改变它们今天没牙**这个事实。）
