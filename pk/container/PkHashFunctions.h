...
//
// **为什么不是 std::hash**：Qt 的 QHash<K,V>/QSet<T> 通过自由函数 `pkHash(K)` 求
// 哈希，而 Krita 全仓有 **18 处自定义 qHash 重载定义**（实测：`qHash` 共 42 处
// 调用/定义，其中 18 处是形如 `uint pkHash(const KoID &id)` 的重载定义）。这些
// 定义散落在 Krita 自己的头文件里，**S 线替换调用点时原样保留**——改用
// std::hash 就等于把这 18 处全部作废，每一处都要人工改写成 std::hash 特化。
//
// 所以默认 hasher 转调 `pkHash(k)`，靠**非限定名字查找 + ADL** 命中它们：
//
...
// pkHash(const PkString&) 与真 Qt 无同名冲突，不受本守卫影响。
...
inline unsigned int pkHash(bool key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key ? 1u : 0u) ^ seed;
}

inline unsigned int pkHash(char key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(static_cast<unsigned char>(key)) ^ seed;
}

inline unsigned int pkHash(signed char key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(static_cast<unsigned char>(key)) ^ seed;
}

inline unsigned int pkHash(unsigned char key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key) ^ seed;
}

inline unsigned int pkHash(short key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(static_cast<unsigned short>(key)) ^ seed;
}

inline unsigned int pkHash(unsigned short key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key) ^ seed;
}

inline unsigned int pkHash(int key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key) ^ seed;
}

inline unsigned int pkHash(unsigned int key, unsigned int seed = 0) noexcept
{
    return key ^ seed;
}

inline unsigned int pkHash(long key, unsigned int seed = 0) noexcept
{
    return pkHashMix64(static_cast<unsigned long long>(static_cast<unsigned long>(key)), seed);
}

inline unsigned int pkHash(unsigned long key, unsigned int seed = 0) noexcept
{
    return pkHashMix64(static_cast<unsigned long long>(key), seed);
}

inline unsigned int pkHash(long long key, unsigned int seed = 0) noexcept
{
    return pkHashMix64(static_cast<unsigned long long>(key), seed);
}

inline unsigned int pkHash(unsigned long long key, unsigned int seed = 0) noexcept
{
    return pkHashMix64(key, seed);
}

inline unsigned int pkHash(char16_t key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key) ^ seed;
}

inline unsigned int pkHash(char32_t key, unsigned int seed = 0) noexcept
{
    return static_cast<unsigned int>(key) ^ seed;
}

inline unsigned int pkHash(wchar_t key, unsigned int seed = 0) noexcept
{
    return pkHashMix64(static_cast<unsigned long long>(key), seed);
}
...
inline unsigned int pkHash(float key, unsigned int seed = 0) noexcept
{
    if (key == 0.0f) {
        return seed;
    }
...
inline unsigned int pkHash(double key, unsigned int seed = 0) noexcept
{
    if (key == 0.0) {
        return seed;
    }
...
template <typename T>
inline unsigned int pkHash(const T *key, unsigned int seed = 0) noexcept
{
...
pkHash(Enum key, unsigned int seed = 0) noexcept
{
    return pkHash(static_cast<typename std::underlying_type<Enum>::type>(key), seed);
}
...
//
// 必须定义在上面那批重载**之后**：`pkHash(k)` 里的 k 是依赖类型，所以查找分两段
// ——模板定义点的普通查找（管内建类型）+ 实例化点的 ADL（管 Krita 自己的类型）。
// 把本结构挪到文件开头，内建类型那一半就会当场失效。
//
// **扩展点只有一个名字：pkHash。** 不给 qHash 留回退——项目目标是移除全部 Qt，
// 再留一个 Qt 名扩展点等于把顺序无关的收益又退回去，还白添一层 SFINAE。
// 喂 `PkHash`/`PkSet` 的自定义重载统一写 `unsigned int pkHash(const X &)`
// （如 `libs/pigment` 的 `KoColorConversionCacheKey`）；那些喂真 Qt `QHash` 的
// `qHash` 重载属于尚未迁移的 Qt 容器路径，与 pk 无关，等容器迁到 PkHash 时一并换名。
//
// **只用一个实参调用**，不传 seed：自定义重载没有第二个形参。
// ---------------------------------------------------------------------------
template <typename K>
struct PkHasher
{
