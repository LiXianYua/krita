...
// _install，QT_VERSION_STR "5.15.7"）的实测探针输出（本任务 probe：qAbs(-0.0)
// signbit=1、qRound(-0.5)=0/qRound(-1.5)=-1/qRound(-2.5)=-2、
// qRound(0.49999999999999994)=1、qFloor(-0.5)=-1/qFloor(-2.7)=-3、
// qCeil(2.3)=3/qCeil(-0.5)=0、qNextPowerOfTwo(0x80000000)=0 等），不是「四舍五入」
...
void PkGlobalCase::absMatchesQt()
{
    ...
    // ⚠ qAbs(INT_MIN) 钉的是「-fwrapv 下定义回绕」这个语义：`-t` 溢出在 shipping
    // 构建里按二补数回绕成 INT_MIN（公式逐字照抄 Qt，Q/P 两侧一致）。本测试 TU
    // 经 pkglobal 的 PUBLIC -fwrapv 编译，去掉 -fwrapv 它才是 UB —— 这条就是
    // 那条旗标的回归守卫。
    PK_COMPARE(qAbs(std::numeric_limits<int>::min()), std::numeric_limits<int>::min());
}
...
void PkGlobalCase::roundFloatOverloadIsReallyFloat()
{
    ...
    PK_COMPARE(qRound(0.49999997), 0);
}
...
void PkGlobalCase::floorCeilMatchQt()
{
    ...
    // qFloor(-0.5)=-1（int(v) 向零截断会给出 0，所以这条是负半值判别式）、
    // qFloor(-2.7)=-3、qCeil(2.3)=3（`-int(-v)` 捷径会给 2，是行为差异）、
    // qCeil(-0.5)=0。
    PK_COMPARE(qFloor(0.0), 0);
    PK_COMPARE(qFloor(2.7), 2);
    PK_COMPARE(qFloor(5.0), 5);
    PK_COMPARE(qFloor(-2.0), -2);
    PK_COMPARE(qFloor(-0.5), -1);
    PK_COMPARE(qFloor(-2.7), -3);

    PK_COMPARE(qCeil(0.0), 0);
    PK_COMPARE(qCeil(2.3), 3);
    PK_COMPARE(qCeil(5.0), 5);
    PK_COMPARE(qCeil(-2.0), -2);
    PK_COMPARE(qCeil(-0.5), 0);
    PK_COMPARE(qCeil(-2.3), -2);
    ...
    PK_COMPARE(qFloor(static_cast<double>(pkFloorNegInf)), pkIntMin);
    PK_COMPARE(qCeil(static_cast<double>(pkFloorPosInf)), pkIntMin);
    PK_COMPARE(qFloor(static_cast<double>(pkFloorPosInf)), pkIntMin);   // int(+inf) UB，x86 → INT_MIN
    PK_COMPARE(qCeil(static_cast<double>(pkFloorNegInf)), pkIntMin);
    PK_COMPARE(qFloor(static_cast<double>(pkFloorNan)), pkIntMin);
    PK_COMPARE(qCeil(static_cast<double>(pkFloorNan)), pkIntMin);
}
...
void PkGlobalCase::nextPowerOfTwoMatchesQt()
{
    ...
    PK_COMPARE(qNextPowerOfTwo(0u), 1u);
    PK_COMPARE(qNextPowerOfTwo(1u), 2u);
    PK_COMPARE(qNextPowerOfTwo(3u), 4u);
    PK_COMPARE(qNextPowerOfTwo(5u), 8u);
    PK_COMPARE(qNextPowerOfTwo(1024u), 2048u);
    PK_COMPARE(qNextPowerOfTwo(0x7fffffffu), 0x80000000u);
    PK_COMPARE(qNextPowerOfTwo(0x40000000u), 0x80000000u);
    // 0x80000000 与 0xFFFFFFFF 都让 OR 级联把低 31 位填满、++v 回绕到 0。
    PK_COMPARE(qNextPowerOfTwo(0x80000000u), 0u);
    PK_COMPARE(qNextPowerOfTwo(0xFFFFFFFFu), 0u);
}
...
void PkGlobalCase::isNaNMatchesQt()
{
    ...
    PK_VERIFY(qIsNaN(std::numeric_limits<float>::quiet_NaN()));
    PK_VERIFY(!qIsNaN(1.0f));
    PK_VERIFY(!qIsNaN(std::numeric_limits<float>::infinity()));
}
...
void PkGlobalCase::qNaNMatchesQt()
{
    ...
    PK_VERIFY(std::isnan(qQNaN()));
    PK_VERIFY(qIsNaN(qQNaN()));
}
