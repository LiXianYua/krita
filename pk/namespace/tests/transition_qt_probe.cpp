// transition_qt_probe.cpp —— R-37「pk namespace Pk 墙让位守卫」的 transition TU 探针。
//
...
//     真 Qt 头在前（R-35 约定）。PkColor.h 经 PkNamespace.h 拉入 pk 的 namespace Pk
//     枚举族——守卫生效后应让位（!QT_CORE_LIB 为假、!QNAMESPACE_H 为假 → 整墙让位），
//     不再与真 Qt qnamespace.h 重定义。static_assert 确认 namespace Pk 保持真 Qt 的
//     （pk 的枚举没漏出来）。
...
// 模式 A：真 Qt 模式下 PkNamespace.h 必须让位：namespace Pk 保持真 Qt 的（KeyboardModifiers
// 是真 Qt QFlags 的 typedef，不是 PkFlags 的）。
static_assert(std::is_same<Pk::KeyboardModifiers, QFlags<Pk::KeyboardModifier>>::value,
              "namespace Pk must be real Qt's under QT_CORE_LIB");
// 真 Qt 位值对拍（让位后来自真 Qt qnamespace.h）。
static_assert(int(Pk::KeyboardModifier::ShiftModifier) == 0x02000000, "real Qt ShiftModifier");
static_assert(int(Pk::MouseButton::LeftButton) == 0x00000001, "real Qt LeftButton");
...
static_assert(int(Pk::KeyboardModifier::ShiftModifier) == 0x02000000, "pk ShiftModifier");
static_assert(int(Pk::MouseButton::LeftButton) == 0x00000001, "pk LeftButton");
static_assert(int(Pk::MouseButton::ExtraButton24) == 0x04000000, "pk MaxMouseButton=ExtraButton24");
static_assert(int(Pk::Key::Key_Escape) == 0x01000000, "pk Key_Escape");
#else
...
int main()
{
    ...
    if (int(Pk::KeyboardModifier::ShiftModifier) != 0x02000000) return 1;
    if (int(Pk::MouseButton::LeftButton) != 0x00000001) return 2;
    if (int(Pk::MouseButton::MaxMouseButton) != 0x04000000) return 3;
#endif
    return 0;
}
