...
void pngReaderPreservesBrushTextMetadata()
{
    // Generated independently with Python's struct/zlib modules.  The two
    // tEXt chunks are literal UTF-8 payloads; no production decoder helper is
    // used to derive the expected metadata or pixels.
    ...
    const PkPngReadResult result = PkPngReader::read(png, sizeof(png));
    expect(!result.image.isNull(), "PNG reader should decode the image payload");
    expect(result.image.width() == 2 && result.image.height() == 1,
           "PNG reader should preserve dimensions");
    expect(result.image.pixel(0, 0) == 0xFF000000u &&
               result.image.pixel(1, 0) == 0xFFFFFFFFu,
           "PNG reader should preserve literal black/white pixels");
    expect(result.text.at("brush_spacing") == "0.375",
           "PNG reader should preserve brush_spacing text");
    expect(result.text.at("brush_name") == u8"画笔名",
           "PNG reader should preserve UTF-8 brush_name text");
}
...
int main()
{
    ...
    pngReaderPreservesBrushTextMetadata();
