...
- **`setContent(PkXmlStreamReader*, bool, ...)` 形状**：**有意打破**
  ...
  这里、12.3 候选 E）三处撞墙后都改用了这个新证据层级。**已裁决
  （2026-08-18，人）**：接受，正式写进 `R线-spec.md`「依赖墙挡住真实测试类
  时」一节，四条硬要求（driver 逐行复刻真实调用点形状 / 校验值来自真 Qt
  探针 / 显式标注非原始文件 / 指名具体依赖墙）本任务三处候选 C/D/E 均已
  核对满足。
...
### 12.5 全分支评审：两条已修复的 Important bug，一条已由人工裁决的判据②问题
...
第三条是判据②本身的证据强度问题，**未由评审单方面判定**，提交给人裁决
（见 12.2 订正段）。**已裁决（2026-08-18）**：接受三处 graft 形状复刻
（候选 C/D/E）作为判据②的合法降级路径，正式写进
`R线-spec.md`「依赖墙挡住真实测试类时」一节。
