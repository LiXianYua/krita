...
//
// 两种编译模式（transition_qt_probe.sh 各编一次、各跑一次）：
//   模式 A（真 Qt 已进 TU）：真 Qt 头在前（R-35 约定），include <QDateTime>（拉
//     qnamespace.h 的 DateFormat）+ PkDateTime.h。守卫生效后 PkDateTime.h 的
//     constexpr ISODate/RFC2822Date/ISODateWithMs 让位，不再与真 Qt 枚举重定义。
...
#if defined(QT_CORE_LIB)
// 真 Qt 模式下 PkDateTime.h 必须让位：Qt::ISODate 保持真 Qt 的 enum DateFormat 值。
