...
#
# 多时区对拍（2026-08-18 裁决，R-16 交接）：QDateTime 工厂函数默认
# timeSpec()==Qt::LocalTime、toString() 按本地墙钟渲染；PkDateTime 已对齐为
# LocalTime（C 库 localtime_r/mktime，读系统 TZ）。本脚本在多个非 UTC 时区下各
# 跑一遍 difftest_time，让「默认 LocalTime vs 固定 UTC」这类差异真的进 mismatch
# 表——早期 `export TZ=UTC` 的做法会把两侧的"本地时区"与"UTC"重合，系统性地
# 遮蔽这类差异（见 R线-spec.md「PkDateTime 时区」一节）。

set -euo pipefail

export LC_ALL=C.UTF-8

# 至少两个非 UTC 时区：一个负偏移（America/Los_Angeles，UTC-8/-7）+ 一个正偏移
# （Asia/Shanghai，UTC+8）。想加时区就往这个数组里追加一行，不用改别处。
TZ_LIST=(
    "America/Los_Angeles"
    "Asia/Shanghai"
)
...
for tz in "${TZ_LIST[@]}"; do
    ...
    echo "==================== TZ=$tz ===================="
    export TZ="$tz"
    # 每个时区单独一个输出文件，方便回看"哪个时区、哪条 DIFFTAG"。
    tz_safe="$(printf '%s' "$tz" | tr '/' '_')"
    # 不能写成「先跑、再拿 $?」：set -e 生效时，命令一旦非 0 退出整个脚本就已经
    # 终止，根本轮不到下一行的 rc=$? 判断——照抄 pk/string/oracle 的 if/! 写法。
    if ! "$OUT/difftest_time" > "$OUT/time.$tz_safe.out" 2>&1; then
        echo "difftest_time 退出码非 0（契约要求 0，即使 mismatch>0；TZ=$tz）" >&2
        exit 1
    fi
    grep -E '^ORACLE-QT ' "$OUT/time.$tz_safe.out"
    grep -E '^DIFFTAG ' "$OUT/time.$tz_safe.out" || true
    grep -E '^DIFF total=' "$OUT/time.$tz_safe.out"
done

echo
echo "出现过的 DIFFTAG 必须在 $HERE/R-16.deviation 里逐条声明过（按每个 TZ 分别核对）。"
