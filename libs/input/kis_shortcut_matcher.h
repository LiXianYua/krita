...
public:
     ...
     * should be eaten by the events filter
     */
    bool autoRepeatedKeyPressed(Pk::Key key);
     ...
     * should be eaten by the events filter
     */
    bool keyReleased(Pk::Key key);

    /**
     * Handles button presses from a tablet or mouse.
     *
     * \param event the event that caused this call.
     * Must carry a tablet or mouse event type.
     *
     * \return whether the event has been handled successfully and
     * should be eaten by the events filter
     */
    bool buttonPressed(Pk::MouseButton button, PkInputEvent *event);
     ...
     * should be eaten by the events filter
     */
    bool buttonReleased(Pk::MouseButton button, PkInputEvent *event);
     ...
     * should be eaten by the events filter
     */
    bool wheelEvent(KisSingleActionShortcut::WheelAction wheelAction, PkInputEvent *event);
     ...
     * should be eaten by the events filter
     */
    bool pointerMoved(PkInputEvent *event);
    ...
    bool touchBeginEvent(PkTouchEvent *event);
    bool touchUpdateEvent(PkTouchEvent *event);
    bool touchEndEvent(PkTouchEvent *event);
     ...
     * all running actions.
     */
    void touchCancelEvent(PkTouchEvent *event);
     ...
     */
    bool touchHoldBeginEvent(PkTouchEvent *event);

    bool nativeGestureBeginEvent(PkNativeGestureEvent *event);
    bool nativeGestureEvent(PkNativeGestureEvent *event);
    bool nativeGestureEndEvent(PkNativeGestureEvent *event);
     ...
     */
    void handlePolledKeys(const PkVector<Pk::Key> &keys);
     ...
     * only.
     */
    bool sanityCheckModifiersCorrectness(Pk::KeyboardModifiers modifiers) const;

    /**
     * Return the internal state of the tracked modifiers.
     */
    PkVector<Pk::Key> debugPressedKeys() const;
     ...
     * forcefully.
     */
    void lostFocusEvent(const PkPointF &localPos);
     ...
     */
    void suppressConflictingKeyActions(const PkVector<PkKeySequence> &shortcuts);
...
private:
    ...
    bool tryRunWheelShortcut(KisSingleActionShortcut::WheelAction wheelAction, PkInputEvent *event);
    template<typename T> bool tryRunSingleActionShortcutImpl(T param, PkInputEvent *event, const PkSet<Pk::Key> &keysState, bool keyboard = true);

    void prepareReadyShortcuts();

    bool tryRunReadyShortcut( Pk::MouseButton button, PkInputEvent* event );
    void tryActivateReadyShortcut();
    bool tryEndRunningShortcut( Pk::MouseButton button, PkInputEvent* event );
    void forceEndRunningShortcut(const PkPointF &localPos);
    void forceDeactivateAllActions();

    void setMaxTouchPointEvent(PkTouchEvent *event);
    void fireReadyTouchShortcut(PkTouchEvent *event);
    KisTouchShortcut *matchTouchShortcut(PkTouchEvent *event);
    bool matchTouchShortcutBasedOnState(PkTouchEvent *event, KisTouchShortcut *shortcut);
    bool tryRunTouchShortcut(PkTouchEvent *event);
    bool tryEndTouchShortcut(PkTouchEvent *event);

    bool tryRunNativeGestureShortcut(PkNativeGestureEvent *event);
    bool tryEndNativeGestureShortcut(PkNativeGestureEvent *event);
