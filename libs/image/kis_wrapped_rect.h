...
struct KisWrappedRect : public PkVector<PkRect> {
    static inline int xToWrappedX(int x, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
    ...
    static inline int yToWrappedY(int y, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
    ...
    static inline PkPoint ptToWrappedPt(PkPoint pt, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
        pt.rx() = xToWrappedX(pt.x(), wrapRect, wrapAxis);
        pt.ry() = yToWrappedY(pt.y(), wrapRect, wrapAxis);
        return pt;
    }

    static inline PkRect clipToWrapRect(PkRect rc, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
        switch (wrapAxis) {
    ...
    static inline bool wrapRectContains(const PkRect &rc, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
        switch (wrapAxis) {
            case WRAPAROUND_HORIZONTAL:
    ...
    static inline bool wrapRectContains(PkPoint pt, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
        switch (wrapAxis) {
            case WRAPAROUND_HORIZONTAL:
     ...
     */
    static inline PkVector<PkPoint> normalizationOriginsForRect(const PkRect &rc, const PkRect &wrapRect, WrapAroundAxis wrapAxis) {
        PkVector<PkPoint> result;
    ...
    static PkVector<PkRect> multiplyWrappedRect(const PkRect &rc,
                                              const PkRect &wrapRect,
                                              const PkRect &limitRect,
        ...
        Q_FOREACH (const PkRect &rect,  wrappedRect) {
            if (rect.isEmpty()) continue;

            for (int y = firstRow; y <= lastRow; y++) {
                for (int x = firstCol; x <= lastCol; x++) {
                    const PkPoint offset(x * wrapRect.width(), y * wrapRect.height());
                    const PkRect currentRect =
...
public:
    ...
    KisWrappedRect(const PkRect &rc, const PkRect &wrapRect, WrapAroundAxis wrapAxis)
        : m_wrapRect(wrapRect),
          m_originalRect(rc)
    {
        if (wrapRectContains(rc, wrapRect, wrapAxis)) {
            ...
            // We add empty rects here because a "splitRect" is expected to contain exactly 4 rects.
            ...
            append(wrapAxis != WRAPAROUND_VERTICAL ?
                clipToWrapRect(newRect.translated(-wrapRect.width(), 0), wrapRect, wrapAxis) : PkRect()); // tr
            append(wrapAxis != WRAPAROUND_HORIZONTAL ?
                clipToWrapRect(newRect.translated(0, -wrapRect.height()), wrapRect, wrapAxis) : PkRect()); // bl
            append(wrapAxis == WRAPAROUND_BOTH ?
                clipToWrapRect(newRect.translated(-wrapRect.width(), -wrapRect.height()), wrapRect, wrapAxis) : PkRect()); // br
        }
    }
    ...
    PkRect topLeft() const {
        return this->at(TOPLEFT);
    }

    PkRect topRight() const {
        return this->at(TOPRIGHT);
    }

    PkRect bottomLeft() const {
        return this->at(BOTTOMLEFT);
    }

    PkRect bottomRight() const {
        return this->at(BOTTOMRIGHT);
    }

    PkRect wrapRect() const {
        return m_wrapRect;
    }

    PkRect originalRect() const {
        return m_originalRect;
    }

private:
    PkRect m_wrapRect;
    PkRect m_originalRect;
};
