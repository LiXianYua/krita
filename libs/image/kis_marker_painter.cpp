...
bool KisMarkerPainter::isRectInValidRange(const PkRect &rect)
{
...
void KisMarkerPainter::fillHalfBrushDiff(const PkPointF &p1, const PkPointF &p2, const PkPointF &p3,
                                         const PkPointF &center, qreal radius)
{
    ...
    PkRectF boundRect(center.x() - fadedRadius, center.y() - fadedRadius,
    ...
    PkRect alignedRect = boundRect.toAlignedRect();
...
void KisMarkerPainter::fillFullCircle(const PkPointF &center, qreal radius)
{
    KoColor currentColor(m_d->color);
...
void KisMarkerPainter::fillCirclesDiff(const PkPointF &c1, qreal r1,
                                       const PkPointF &c2, qreal r2)
{
    PkVector<PkPointF> n = KisAlgebra2D::intersectTwoCircles(c1, r1, c2, r2);

    if (n.size() < 2) {
        fillFullCircle(c2, r2);
    } else {
        const PkPointF diff = c2 - c1;
        const qreal normDiffInv = 1.0 / KisAlgebra2D::norm(diff);
        const PkPointF direction = diff * normDiffInv;
        const PkPointF p = c1 + r1 * direction;
        const PkPointF q = c2 + r2 * direction;
