...
struct KoLabTraits : public KoColorSpaceTrait<_channels_type_, 4, 3> {
    ...
    inline static PkString normalisedChannelValueText(const quint8 *pixel, quint32 channelIndex)
    {
        ...
        switch (channelIndex) {
        case L_pos:
            return PkString("%1").arg(100.0 * pkBound((qreal)0, ((qreal)c) / math_trait::unitValueL, (qreal)math_trait::unitValueL));
        case a_pos:
        case b_pos:
        ...
        case 3:
            return PkString("%1").arg(100.0 * pkBound((qreal)0, ((qreal)c) / math_trait::unitValue, (qreal)math_trait::unitValue));
        default:
            return PkString("Error");
        }
    }
    ...
    inline static void fromNormalisedChannelsValue(quint8 *pixel, const PkVector<float> &values)
    {
        ...
        for (uint i = 0; i < parent::channels_nb; i++) {
            ...
            switch (i) {
            case L_pos:
                b = pkBound((float)math_trait::zeroValueL,
