...
KisStandardBrushSizes::KisStandardBrushSizes(int minSize, int maxSize)
{
    ...
    do {
        ...
        const int increment = pkMax(1, int(std::ceil(qreal(brushSize) / 15)));
        brushSize += increment;
...
{
    const auto result = std::upper_bound(m_sizes.begin(), m_sizes.end(), pkRound(size));
    return result != m_sizes.end() ? *result : m_sizes.back();
}

int KisStandardBrushSizes::decreaseBrushSize(qreal size)
{
    const auto result = std::upper_bound(m_sizes.rbegin(), m_sizes.rend(), pkRound(size), std::greater<int>());
