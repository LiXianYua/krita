...
KisPngCodec::KisPngCodec(const KisPngCodecContext &context)
    : m_context(context)
{
    m_image = 0;
}

KisPngCodec::~KisPngCodec()
{
}
...
KisImportExportErrorCode KisPngCodec::buildImage(QIODevice* iod)
{
    ...
    else if (color_nb_bits == 16 && !fromBlender && m_context.importProfilePolicy) {
    ...
    if (m_image == 0) {
        KisUndoStore *store = m_context.documentContext
            ? m_context.documentContext->createUndoStore()
            : new KisSurrogateUndoStore();
...
KisImageSP KisPngCodec::image() const
{
    return m_image;
}
...
void KisPngCodec::cancel()
{
    m_stop = true;
}

bool KisPngCodec::isColorSpaceSupported(const KoColorSpace *cs)
{
