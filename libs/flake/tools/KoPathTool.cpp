...
{
    // `actionCollectionObject()` 而不是 `actionCollection()`：后者只存在于 qt 支
    // （返回真 `QObject*`），本文件在 native 桶里编。取回的是桶无关的
    // `PkObject *`，`findChild` 走 PkObject 的对象树。
    KoCanvasController *controller = canvas ? canvas->canvasController() : nullptr;
    PkObject *collection = controller ? controller->actionCollectionObject() : nullptr;
    return collection ? collection->findChild<QAction *>(name) : nullptr;
}
}
    ...
    , m_textOutlineHelper(new KoSvgTextShapeOutlineHelper(canvas))
{
    ...
    // 这条分支上退回 QCursor(Pk::CrossCursor)/QCursor(Pk::SizeAllCursor)，那个形状
...
void KoPathTool::pointTypeChanged(KoPathPointTypeCommand::PointType type)
{
    ...
    if (m_pointSelection.hasSelection()) {
        PkList<KoPathPointData> selectedPoints = m_pointSelection.selectedPointsData();
...
{
    Q_D(KoToolBase);
    PkList<KoPathPointData> segments(m_pointSelection.selectedSegmentsData());
    if (segments.size() == 1) {
...
void KoPathTool::pointToLine()
{
    ...
    if (m_pointSelection.hasSelection()) {
        PkList<KoPathPointData> selectedPoints = m_pointSelection.selectedPointsData();
        PkList<KoPathPointData> pointToChange;

        PkList<KoPathPointData>::const_iterator it(selectedPoints.constBegin());
...
KUndo2Command* KoPathTool::createPointToCurveCommand(const PkList<KoPathPointData> &points)
{
    KUndo2Command *command = 0;
    PkList<KoPathPointData> pointToChange;

    PkList<KoPathPointData>::const_iterator it(points.constBegin());
...
void KoPathTool::convertToPath()
{
    ...
    PkList<KoParameterShape*> parameterShapes;

    for (KoShape *shape : m_pointSelection.selectedShapes()) {
    ...
    PkList<KoSvgTextShape*> textShapes;
    for (KoShape *shape : selection->selectedEditableShapes()) {
    ...
    if (!textShapes.isEmpty()) {
        ...
        const PkList<KoShape*> oldSelectedShapes = implicitCastList<KoShape*>(textShapes);
        ...
        for (KoSvgTextShape *shape : textShapes) {
...
void KoPathTool::mergePointsImpl(bool doJoin)
{
    ...
    PkList<KoPathPointData> pointData = m_pointSelection.selectedPointsData();
...
void KoPathTool::paint(PkPainter &painter, const KoViewConverter &converter)
{
    ...
    for (KoPathShape *shape : m_pointSelection.selectedShapes()) {
...
PkRectF KoPathTool::decorationsRect() const
{
    const_cast<KoPathToolSelection&>(m_pointSelection).update();

    PkRectF newDecorationsRect;

    for (KoShape *shape : m_pointSelection.selectedShapes()) {
        newDecorationsRect |= kisGrowRect(shape->boundingRect(), handleDocRadius());
    }

    for (const KoPathPoint *point : m_pointSelection.selectedPoints()) {
    ...
    if (m_activeSegment) {
        ...
        PkRectF rect = segment.boundingRect();
    ...
    if (m_activeHandle) {
        ...
        if (event->button() & Pk::LeftButton) {
            ...
            if (m_activeSegment && m_activeSegment->isValid()) {
                ...
                m_pointSelection.add(segment.first(), !(event->modifiers() & Pk::ShiftModifier));
...
void KoPathTool::mouseMoveEvent(KoPointerEvent *event)
{
    ...
    for (KoPathShape *shape : m_pointSelection.selectedShapes()) {
        PkRectF roi = shape->documentToShape(handleGrabRect(event->point));
        KoParameterShape * parameterShape = dynamic_cast<KoParameterShape*>(shape);
        if (parameterShape && parameterShape->isParametricShape()) {
            ...
            PkList<KoPathPoint*> points = shape->pointsAt(roi, true);
            if (! points.empty()) {
...
KoPathTool::PathSegment* KoPathTool::segmentAtPoint(const PkPointF &point)
{
    // the max allowed distance from a segment
    const PkRectF grabRoi = handleGrabRect(point);
...
void KoPathTool::activate(const PkSet<KoShape*> &shapes)
{
    ...
    const PkPointer<KoPathTool> toolGuard(this);
    const auto connectAction = [this, toolGuard](QAction *action, auto method) {
        m_actionConnections.append(PkObject::connect(action, &QAction::triggered, action, [toolGuard, method] {
            if (toolGuard) (toolGuard.data()->*method)();
        }));
    };
    connectAction(m_actionCurvePoint, &KoPathTool::pointToCurve);
    connectAction(m_actionLinePoint, &KoPathTool::pointToLine);
    connectAction(m_actionLineSegment, &KoPathTool::segmentToLine);
    connectAction(m_actionCurveSegment, &KoPathTool::segmentToCurve);
    connectAction(m_actionAddPoint, &KoPathTool::insertPoints);
    connectAction(m_actionRemovePoint, &KoPathTool::removePoints);
    connectAction(m_actionBreakPoint, &KoPathTool::breakAtPoint);
    connectAction(m_actionBreakSegment, &KoPathTool::breakAtSegment);
    connectAction(m_actionBreakSelection, &KoPathTool::breakAtSelection);
    connectAction(m_actionJoinSegment, &KoPathTool::joinPoints);
    connectAction(m_actionMergePoints, &KoPathTool::mergePoints);
    connectAction(m_actionConvertToPath, &KoPathTool::convertToPath);
    connectAction(m_actionPathPointCorner, &KoPathTool::pointTypeChangedCorner);
    connectAction(m_actionPathPointSmooth, &KoPathTool::pointTypeChangedSmooth);
    connectAction(m_actionPathPointSymmetric, &KoPathTool::pointTypeChangedSymmetric);
    PkObject::connect(&m_pointSelection, &KoPathToolSelection::selectionChanged,
...
void KoPathTool::initializeWithShapes(const PkList<KoShape*> shapes)
{
    PkList<KoPathShape*> selectedShapes;
...
void KoPathTool::updateActions()
{
    ...
    if (!pointData.isEmpty()) {
        for (const KoPathPointData &pd : pointData) {
    ...
    if (!segments.isEmpty()) {
        ...
        for (const KoPathPointData &pd : segments) {
...
void KoPathTool::deactivate()
{
    ...
    m_pointSelection.clear();
    m_pointSelection.setSelectedShapes(PkList<KoPathShape*>());
    m_activeHandle.reset();
    m_activeSegment.reset();
    m_currentStrategy.reset();
    d->canvas->snapGuide()->reset();

    // `PkObject::disconnect` 取非 const 引用（断开后句柄要置活标志），
    // 所以这里按引用遍历而不是 const 引用。
    for (PkConnection &connection : m_actionConnections) {
...
void KoPathTool::canvasResourceChanged(int key, const PkVariant & /*res*/)
{
