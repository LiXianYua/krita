...
#include "pk/geometry/PkVectorND.h"
#include "pk/geometry/PkMatrix4x4.h"
...
struct KRITASURFACECOLORMANAGEMENTAPI_EXPORT XYZ {
    ...
    xyY toxyY() const;
    xy toxy() const;
    PkVector3D asVector() const;
    XYZ operator*(double factor) const;
    XYZ operator/(double factor) const;
    XYZ operator+(const XYZ &other) const;
    bool operator==(const XYZ &other) const;

    static XYZ fromVector(const PkVector3D &vector);
};
...
public:
     ...
     */
    static PkMatrix4x4 chromaticAdaptationMatrix(XYZ sourceWhitepoint, XYZ destinationWhitepoint);

    static PkMatrix4x4 calculateToXYZMatrix(XYZ red, XYZ green, XYZ blue, XYZ white);
     ...
     */
    const PkMatrix4x4 &fromXYZ() const;
    PkMatrix4x4 toLMS() const;
    PkMatrix4x4 fromLMS() const;
    ...
    PkMatrix4x4 relativeColorimetricTo(const Colorimetry &other) const;
    PkMatrix4x4 absoluteColorimetricTo(const Colorimetry &other) const;
...
private:
    ...
    XYZ m_white;
    PkMatrix4x4 m_toXYZ;
    PkMatrix4x4 m_fromXYZ;
};

KRITASURFACECOLORMANAGEMENTAPI_EXPORT PkDebug operator<<(PkDebug debug, const xy &value);
KRITASURFACECOLORMANAGEMENTAPI_EXPORT PkDebug operator<<(PkDebug debug, const xyY &value);
KRITASURFACECOLORMANAGEMENTAPI_EXPORT PkDebug operator<<(PkDebug debug, const XYZ &value);
KRITASURFACECOLORMANAGEMENTAPI_EXPORT PkDebug operator<<(PkDebug debug, const Colorimetry &value);
