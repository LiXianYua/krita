...
- **不要用 `SendMessage` 找派你的人。** `task-agent` 是**类型名不是地址**，解析不到；
  嵌套 agent 之间那条通道本来就不通。实测有实现者反复尝试
  `SendMessage({to: "task-agent"})`，最后在报告开头写「由上层会话转达」——
  **通道一直是通的，那句话是白费的力气加误导性的措辞。**
- **不要在报告里写「请上层转达」。** 没有人需要转达。
- **最终输出要装的是报告本身，不是关于通道的说明。** 实测踩过：一个子 agent 跑完
  52 次工具调用、报告写进了文件、提交也在，**最终输出却整段只在解释「`task-agent`
  是类型名不是地址、我的输出就是返回值」——返回值里一条结果都没有**，派它的人
  拿到一份空报告。**上面这几条是给你自己看的纪律，不是给派你的人看的汇报内容。**
  发现自己在最终输出里谈论 `SendMessage`、返回值机制、或本节任何内容，**删掉，
  换成结果。**
...
- **不要预告「什么东西预期会消失/失败」**。那等于提前替判据签字：真的漏删了，
  它也会当成预期之内。
...
| 谁 | 错法 | 后果 |
|---|---|---|
| 主会话 | 派 D-02-a 时把 `W1` 注解成「删」 | 三条指令方向全反，其中「基线 `plugins-impex-*` 消失是预期的」会**预先合理化**判据 4 要防的事——真删掉 `.kra` 支持也编得过、测得绿。被 task-agent 自己去读决策文档才发现 |
| `task-agent` | 派 Task 9 时复述 `squareToQuad`/`quadToSquare` 的出现次数，名字与数值错位 | 实现者数对了、归因错了，写成「计划有误」。**计划原文是对的** |
...
实测代价：D-01b 用它等子 agent，88 次空转、**总工具调用 639**，而同期任务是
82～134。约 500 次纯浪费，产出为零。
...
段与段之间正常返回。实测 298 个测试分 `1,219` + `220,298` 两段跑完。
**不要为了「跑得完」就放后台**——实测有子 agent 这么做，回合一结束后台 ctest
就被杀，只跑到 64/298，整段作废重来。
