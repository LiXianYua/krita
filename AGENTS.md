...
定义——都在启动 prompt 里给出（调用 `drive` skill 的主会话派发时给的）。**你不挑任务、
...
- **不要用执行器的发消息动作找派你的人。** `task-agent` 是**类型名不是地址**，解析不到。
  你不需要它——通道本来就是通的。
- **不要在报告里写「请上层转达」。** 没有人需要转达。
- **最终输出要装的是报告本身，不是关于通道的说明。** 发现自己在最终输出里谈论
  发消息动作、返回值机制、或本节任何内容，**删掉，换成结果**——**上面这几条是
...
| 情形 | 做法 |
|---|---|
| 要派多个 agent 并行 | **在同一轮发多个前台 agent 调用**——它们并发执行，每个都是阻塞的，返回值一起给你。**没有「要等」这回事** |
| 等执行器派出的子 agent | **阻塞在该执行器的前台派发动作上**（不要转为后台）。**不要结束回合去等**——见下 |
| 等 **你自己用 shell 起的进程**（`ctest`、全量 `ninja`） | **必须前台跑（阻塞的 shell 调用）。** 你一结束回合，你自己的会话就结束了，**后台进程跟着被杀** |
| 想看进度 | 让长命令把进度写进文件，回来一次性读。不要边跑边查 |

**全量 `ctest` 跑不完一次 shell 调用的超时怎么办**：`-I <起>,<止>` 分段，**每段仍然前台跑**，
段与段之间正常返回。先 `ctest -N | tail -1` 拿到总数再决定切几段。
**不要为了「跑得完」就放后台**——你一结束回合，你的会话就结束了，后台 ctest 跟着
被杀，跑了一半的结果全部作废。

**阻塞等是零成本，轮询等是每轮全量重发**——这是 0 与 N² 的差别。
**写了 `sleep`/`while`/`until`/`kill -0` 的等待循环就是错的，删掉，改成阻塞调用。**

### ⛔ 「结束回合去等子 agent」会把结果送到别人手上

**你结束回合，子 agent 的完成通知只有主会话收得到——你收不到，也不会自己醒。**
得由人把内容转述给你、再唤醒你：一来一回，而且转述必然比一手信息少。
**你以为自己在等，实际上你已经出局了。**

所以「后台派发 + 自己守着」的**所有外壳都不要**：转为后台再轮询 ·
另起监控动作盯构建 · 反复查文件看它写完没有 · 任何「我先看看它好了没」。

**要并行就一条消息里发多个阻塞调用**（表里第一行）——**那已经是并行了，不需要后台。**

> **这条对 `subagent-driven-development` 的修复轮同样成立**：技能写的是「轮 1–3 唤醒
> 原实现者」，但唤醒是异步的、返回值同样只到主会话。技能自己给了替代路径——
> 送不到就**派新实现者**，带上 brief 路径、报告文件路径、待修的发现，
> 「the report file is the persistent memory either way」。**持久记忆在报告文件里，
> 不在那个 agent 的上下文里。**
...
- 改 `$PK/.exec/`、执行器配置目录下任何文件，或 `$PK/AGENTS.md`——流水线
  本身、角色定义、工作空间约定
- 改本文件
- 修改不属于你当前任务的 target
...
验证结果是交付物，不是只在当前 agent 上下文里消费的一次性动作。每次构建、测试、扫描或
benchmark 都要在报告中登记源码/配置提交（或 tree）指纹、实际命令、环境要点、退出码与
结论。后续 reviewer、修复 agent 和父会话先读并复用这份证据；不得因为“独立”就机械重跑
同一套全量命令。

只有源码或 CMake 改动触及测试闭包、测试夹具/工具链/运行环境变化，或新发现直接质疑该证据
的前提时，既有证据才失效；重跑受影响的最小子集。任务收尾的全量构建/测试是唯一的全量闸门，
其余轮次只跑新增 diff 和新增风险覆盖。报告必须区分“复用证据”和“本轮新增验证”，并说明
失效原因。

## 完成判定

以 `$PK/.exec/roles/task-agent.md`「完成判定」一节为准，这里不重复，避免两份
...
| 那一段 | 防什么 |
|---|---|
| `source .../krita-ci-env/env` | 前缀里的 Qt/KF5/ICU 等运行时库。**shell 工具不跨调用保留环境变量**，上次 `source` 过不算数——掉了它会**全部测试瞬间全红**报 `libicui18n.so.70` 找不到 |
...
不要裸跑 shell 命令后整段读回来。**你的上下文每轮全量重发**，一次几百行的 grep 输出
会在你剩下的每一个 turn 里重复计费。

写文件仍然用当前执行器的文件编辑能力——`ctx_execute*` 是沙箱，写不进磁盘。
...
**已知的坑列在 `$PK/AGENTS.md`「报任何数字必须带口径」一节，开工前读那一份。**
