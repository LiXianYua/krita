...
const PkString SPRAYSHAPE_ENABLED = "SprayShape/enabled";
const PkString SPRAYSHAPE_SHAPE = "SprayShape/shape";
const PkString SPRAYSHAPE_PROPORTIONAL = "SprayShape/proportional";
const PkString SPRAYSHAPE_WIDTH = "SprayShape/width";
const PkString SPRAYSHAPE_HEIGHT = "SprayShape/height";
const PkString SPRAYSHAPE_IMAGE_URL = "SprayShape/imageUrl";
const PkString SPRAYSHAPE_USE_ASPECT = "SprayShape/useAspect";
...
bool KisSprayShapeOptionData::read(const KisPropertiesConfiguration *settings)
{
	...
	PkString url = settings->getString(SPRAYSHAPE_IMAGE_URL);
	if (url.isEmpty()) {
		image = PkImage();
	}
	else {
...
PkSize KisSprayShapeOptionData::effectiveSize(int diameter, qreal scale) const
{
