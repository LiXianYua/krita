...
PkVector<KoShapeMeshGradientHandles::Handle> KoShapeMeshGradientHandles::handles() const
{
    ...
    PkTransform t = absoluteTransformation(g->gradientUnits());
                                                              ...
                                                              const PkPointF &newPos)
{
    ...
    PkScopedPointer<SvgMeshGradient> newGradient(new SvgMeshGradient(*wrapper.meshgradient()));
    ...
    std::array<PkPointF, 4> path = patch->getSegment(handle.segmentType);

    PkTransform t = absoluteTransformation(newGradient->gradientUnits()).inverted();
    ...
    return wrapper.setMeshGradient(newGradient.data(), PkTransform());
}

PkPainterPath KoShapeMeshGradientHandles::path() const
{
    ...
    PkScopedPointer<SvgMeshGradient> g(new SvgMeshGradient(*gradient()));
    if (g->gradientUnits() == KoFlake::ObjectBoundingBox) {
        const PkTransform gradientToUser = KisAlgebra2D::mapToRect(m_shape->outlineRect());
...
PkVector<PkPainterPath> KoShapeMeshGradientHandles::getConnectedPath(const Handle &handle) const
{
    KIS_ASSERT(handle.type != Handle::None);

    PkVector<PkPainterPath> result;

    // TODO(sh_zam): Handle OBB and user mode in and only in SvgMeshPatch
    const PkTransform t = (gradient()->gradientUnits() == KoFlake::ObjectBoundingBox)
    ...
    if (handle.type == Handle::BezierHandle) {
        ...
        std::transform(path.begin(), path.end(), path.begin(), [&t](PkPointF &point) { return t.map(point); });
        painterPath.moveTo(path[0]);
        painterPath.cubicTo(path[1], path[2], path[3]);
        result << painterPath;
    } else {
        PkVector<SvgMeshPosition> positions = mesharray->getConnectedPaths(handle.getPosition());
        for (const auto &position: positions) {
            SvgMeshPath path = mesharray->getPath(position);
            std::transform(path.begin(), path.end(), path.begin(), [&t](PkPointF &point) { return t.map(point); });
            painterPath = PkPainterPath();
            painterPath.moveTo(path[0]);
            painterPath.cubicTo(path[1], path[2], path[3]);
            result << painterPath;
        }
...
PkPointF KoShapeMeshGradientHandles::getAttachedCorner(const Handle &bezierHandle) const
{
...
PkVector<KoShapeMeshGradientHandles::Handle> KoShapeMeshGradientHandles::getHandles(const SvgMeshArray *mesharray,
                                                                                   SvgMeshPatch::Type type,
                                                                                   int row,
                                                                                   int col) const
{
    PkVector<Handle> buffer;
    std::array<PkPointF, 4> path = mesharray->getPath(type, row, col);
...
PkVector<KoShapeMeshGradientHandles::Handle> KoShapeMeshGradientHandles::getBezierHandles(const SvgMeshArray *mesharray,
                                                                                         SvgMeshPatch::Type type,
                                                                                         int row,
                                                                                         int col) const
{
...
PkTransform KoShapeMeshGradientHandles::absoluteTransformation(KoFlake::CoordinateSystem system) const
{
    PkTransform t;
